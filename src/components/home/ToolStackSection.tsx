...
const toolCardSize: Record<ToolItem["size"], string> = {
  wide: "md:col-span-2",
  tall: "md:row-span-2",
  compact: "",
};
        ...
        <div className="grid auto-rows-[minmax(11rem,auto)] grid-cols-1 gap-4 md:grid-cols-4">
          {tools.map((tool, index) => (
            <motion.article
              key={tool.name}
              className={[
                "group flex min-w-0 flex-col justify-between overflow-hidden rounded-2xl border border-gray-200 bg-white p-5 shadow-sm shadow-gray-950/5 transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/10",
                toolCardSize[tool.size],
              ].join(" ")}
            ...
            >
              <div className="flex min-w-0 items-start justify-between gap-4">
                ...
                <ArrowUpRight className="mt-1 h-4 w-4 shrink-0 text-gray-300 transition-colors group-hover:text-primary" />
              </div>
              ...
              <div className="mt-6 min-w-0">
                ...
                <p className="mt-2 text-sm leading-6 text-gray-600">{tool.role}</p>
